...
ll dp[1001][2][2][2];
ll find(vector<pair<ll,vector<ll>>> &v, ll a, ll b, ll c, ll ind)
{
    if(a==0 && b == 0 && c == 0)
    return 0;
    if(ind==v.size())
    ...
    ll a1=0,b1=0;
    a1 = v[ind].first + find(v,a-min(a,v[ind].second[0]), b-min(b,v[ind].second[1]),c-min(c,v[ind].second[2]),ind+1);
    b1 = find(v,a,b,c,ind+1);
    return dp[ind][a][b][c] = min(a1,b1);
}
signed main() 
{
